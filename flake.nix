{
  ...
  inputs = {
    ...
    primer.url = github:hackworthltd/primer/73eec600ead409ef9fd5cc3b26417c24541f7f90;
  };
  ...
  outputs =
    ...
    let
      ...
      # All systems we support in this flake.
      allSystems = [
        "x86_64-linux"
        "aarch64-linux"
        "aarch64-darwin"
      ...
      # All systems for which we build the frontend. Currently we
      # don't include "aarch64-linux" because some of the dev
      # dependencies won't build on that platform.
      allPrimerAppSystems = [
        "x86_64-linux"
        "aarch64-darwin"
      ];

      forAllSupportedSystems = flake-utils.lib.eachSystem allSystems;
      ...
      primerAppFlake = dream2nix.lib.makeFlakeOutputs {
        systems = allPrimerAppSystems;
        config.projectRoot = ./.;
        source = src;
        settings = [
          {
